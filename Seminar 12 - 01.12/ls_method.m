...
for k = 1 : n_par
    for j = 1 : n_par
        A(k, j) = sum(arrayfun(@(x) f{j}(x) .* f{k}(x), x_val));
    end
    b(k) = sum(y_val .* arrayfun(f{k}, x_val));
end

par = linsolve(A, b);
table(f.', par_true, par, abs(par_true - par),...
...
y_est = zeros(size(x));
for k = 1 : n_par
    y_est = y_est + par(k) * arrayfun(f{k}, x);
end
